...
const OutputItems = () => {
  ...
  const handleSave = async () => {
    ...
    const url = itemId
      ? `http://http://orion.vps-kinghost.net:3001/api/saida-item/${itemId}`
      : 'http://http://orion.vps-kinghost.net:3001/api/saida-item';
  ...
  const handleProdutoIdChange = async (event) => {
    ...
    if (produtoId) {
      try {
        let response = await fetch(`http://http://orion.vps-kinghost.net:3001/api/saida-item/produtoId/${produtoId}`);
        let data = await response.json();

        if (!data) {
          response = await fetch(`http://http://orion.vps-kinghost.net:3001/api/saida-item/descricao/${produtoId}`);
  ...
  useEffect(() => {
    if (itemId) {
      const fetchData = async () => {
        try {
          const response = await fetch(`http://http://orion.vps-kinghost.net:3001/api/saida-item/${itemId}`);
