...
const RegisterUser = () => {
  ...
  useEffect(() => {
    if (itemId) {
      const fetchData = async () => {
        try {
          const response = await fetch(`http://http://orion.vps-kinghost.net:3001/api/usuario/${itemId}`);
  ...
  const handleSave = async () => {
    ...
    const url = itemId
      ? `http://http://orion.vps-kinghost.net:3001/api/usuario/${itemId}`
      : 'http://http://orion.vps-kinghost.net:3001/api/usuario';
