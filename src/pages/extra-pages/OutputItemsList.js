...
const OutputItemsList = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await fetch('http://http://orion.vps-kinghost.net:3001/api/saida-item');
  ...
  const deleteItem = async (id) => {
    try {
      await fetch(`http://http://orion.vps-kinghost.net:3001/api/saida-item/${id}`, { method: 'DELETE' });
      const response = await fetch('http://http://orion.vps-kinghost.net:3001/api/saida-item');
      const data = await response.json();
      setOuputItems(data);
    } catch (error) {
