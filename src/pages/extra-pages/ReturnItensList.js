...
const ReturnItensList = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await fetch('http://http://orion.vps-kinghost.net:3001/api/devolucao-item');
  ...
  const deleteItem = async (id) => {
    try {
      await fetch(`http://http://orion.vps-kinghost.net:3001/api/devolucao-item/${id}`, { method: 'DELETE' });
      const response = await fetch('http://http://orion.vps-kinghost.net:3001/api/devolucao-item');
      const data = await response.json();
      setReturnItens(data);
    } catch (error) {
